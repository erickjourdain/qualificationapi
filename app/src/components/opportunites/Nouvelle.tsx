...
const Nouvelle = () => {
  ...
  useBlocker({
    blockerFn: () => window.confirm("Les données n'ont pas été sauvegardée! Souhaitez-vous continuez?"),
    condition: isDirty,
  });
  ...
  const {
    ...
    onSuccess: () => {
      setAlerte({ severite: "success", message: "L'opportunité a été créée avec succès" });
        ...
        >
          ...
          <Box mt={3}>
            <Stack spacing={2} direction="row">
              <Button variant="contained" color="primary" disabled={isPending} onClick={handleSubmit(onSubmit)}>
                Enregistrer
              </Button>
              <Button variant="contained" color="warning" disabled={isPending} onClick={() => reset()}>
                Reset
              </Button>
              <Button variant="contained" color="secondary" disabled={isPending} onClick={onFinish}>
                Annuler
              </Button>
            </Stack>
          </Box>
