...
export const Route = createFileRoute('/_mainLayout/_auth/opportunites/')({  
  component: Liste,
  ...
  loader: async ({ deps, context }) => {
    const { data } = await context.queryClient.fetchQuery({
      ...
      queryFn: () => {
        let filter = "";
        const include = ["id", "uuid", "societe", "nom", "prenom", "createur", "projet", "opportunite", "createdAt"];
        if (deps.search && deps.search.length) {
          filter = sfOr([sfLike("projet", deps.search), sfLike("opportunite", deps.search), sfLike("societe", deps.search)]).toString();
        }
