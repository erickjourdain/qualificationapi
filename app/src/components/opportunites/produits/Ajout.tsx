...
import { Route as oppRoute } from "@/routes/_mainLayout/_auth/opportunites/$uuid";
...
const ProduitAdd = ({ onClose }: ProduitAddtProps) => {
  ...
  const header = oppRoute.useLoaderData();
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: Inputs) => createProduit({ header: header.id, description: data.description }),
    onSuccess: () => {
