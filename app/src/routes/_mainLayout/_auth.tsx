...
function Auth() {
  ...
  const { data, hasNextPage, fetchNextPage, error, isLoading, isError } = useInfiniteQuery({
    ...
    initialPageParam: 1,
    getNextPageParam: (forms: FormsAPI) => (forms.hasNext) ? forms.page + 1 : undefined,
  });

  // Mise à jour du tableau des produits suite récupération des données
  useEffect(() => {
    const forms: FormAPI[] = [];
    data?.pages.map(f => f.data.map(data => {
      forms.push(data)
    }))
